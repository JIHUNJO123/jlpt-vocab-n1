...
class _SentenceQuizScreenState extends State<SentenceQuizScreen> {
  ...
  Future<void> _loadWords() async {
    ...
    // 예문이 있는 단어만 필터링
    List<Word> words = jsonWords.where((w) {
      final hasExample = w.example.isNotEmpty && w.example.length > 10;
      final wordInExample = w.example.contains(w.word) || 
    ...
    if (translationService.needsTranslation) {
      for (var word in words) {
        final embeddedTranslation = word.getEmbeddedTranslation(langCode, 'definition');
  ...
  void _generateQuestion() {
    ...
    // 정답 (표시용 단어)
    final correctAnswer = currentWord.kanji ?? currentWord.hiragana ?? currentWord.word;
    ...
    // 같은 품사 단어들 우선
    final samePosWords = _words.where((w) => 
    ...
    while (wrongAnswers.length < 3) {
      ...
      final wrongAnswer = wrongWord.kanji ?? wrongWord.hiragana ?? wrongWord.word;
  ...
  Widget build(BuildContext context) {
    ...
    final currentWord = _quizWords[_currentIndex];
    final correctAnswer = currentWord.kanji ?? currentWord.hiragana ?? currentWord.word;
    final definition = _translatedDefinitions[currentWord.id] ?? currentWord.definition;
                            ...
                            if (_answered) ...[
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  if (currentWord.hiragana != null && currentWord.kanji != null)
                                    Text(
